...
def signin(request):
    if request.method == "GET":
        ...
        if user is None:
            return render(request, "signin.html", {"form": AuthenticationForm, "error": "password or username is incorrect"})
        else:
            login(request, user)
            return redirect("tasks")
...
def create_task(request):
    if request.method == 'GET':
        ...
        try:
            ...
            return render(request, "create_task.html", {'form': TaskForm,'error':'por favor provee un dato valido'})
